...
- 市场数据：选择**允许个人使用且稳定的公开接口**；若不可避免使用第三方抓取，需加缓存与限频，并预留替换接口层。
...
1) “每天 9 点”指哪个时区（本地/中国/UTC）？
2) 市场数据允许使用的来源偏好：更偏官方/交易所、还是接受稳定第三方（如聚合行情）？
3) 是否必须离线可用（打开 `site/index.html` 即可），还是接受需要 API 请求的在线站点？
4) 英文翻译：是否强依赖 OpenAI（有 key），还是要无 key 的降级方案？
